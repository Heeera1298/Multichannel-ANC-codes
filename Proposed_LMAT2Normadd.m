...
delta2=0.1;
N     =      1e5;
Avg   =     1;  % Number of Realizations
 %x1    = stblrnd(1.85,beta,gam,delta,N,Avg); 
 load("input_185.mat")
 Ref_Signal=x1';
mew=[1e-1 1e-2 1e-3 1e-4 1e-6 1e-7];
lambda=0.9999;
delta=0.3;
...
for n=1:N
    ...
    y2 = [w2'*xx(1:Lw)'    y2(1:M-1)];
       ...
       xh22=[sh22'*xx(1:M)' xh22(1:L-1)];
        ...
        w1  = w1 + ( mew1 * (e1a(n)^2)*sign(e1a(n)) ) * xh11(1:Lw)'+ ( mew2 * (e2a(n)^2)*sign(e2a(n)) ) * xh12(1:Lw)';
         w2  = w2 + ( mew1 * (e1a(n)^2)*sign(e1a(n)) ) * xh21(1:Lw)'+ ( mew2 * (e2a(n)^2)*sign(e2a(n)) ) * xh22(1:Lw)';
