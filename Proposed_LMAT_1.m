...
%          mew1 = mew(b)  / (+delta2+E1(n+1));
%           mew2 = mew(b)  / ((norm(xh21+xh22)^2)+delta2+E2(n+1));
        mu1= mew(b)/(1 + beta2* (abs(e1a(n))^3)+delta2+E2(n+1));
        mu2= mew(b)/(1 + beta2* (abs(e2a(n))^3)+delta2+E2(n+1));
        w1  = w1 + (( mu1 * (e1a(n)^2)*sign(e1a(n)) ) * xh11(1:Lw)')/((norm(xh11+xh12)^2)+delta2+E1(n+1))+ (( mu2 * (e2a(n)^2)*sign(e2a(n)) ) * xh12(1:Lw)')/((norm(xh11+xh12)^2)+delta2+E1(n+1));
         w2  = w2 + (( mu1 * (e1a(n)^2)*sign(e1a(n)) ) * xh21(1:Lw)')/((norm(xh21+xh22)^2)+delta2+E1(n+1))+ ( ( mu2 * (e2a(n)^2)*sign(e2a(n)) ) * xh22(1:Lw)')/((norm(xh21+xh22)^2)+delta2+E1(n+1));
end
   E1h = E1h + (abs(e1a));
    EE1h(avg,:) = e1a;
    D1 = D1 + (abs(d1));
